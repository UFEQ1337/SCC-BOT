...
function showFinalMessage({ removedCount, duration }) {
    const modalText = document.getElementById("progressText");
    let message = `Usunięto użytkowników: ${removedCount}<br>`;
...
async function removeAgentFromQueue(websiteUrl, apiKey, tenantName, queueId, extensionNumber) {
    const url = `${websiteUrl}/pbx/proxyapi.php?key=${apiKey}&reqtype=QUEUE&tenant=${tenantName}&id=${queueId}&extension=${extensionNumber}&action=del`;
    console.log(`Usuwanie agenta: URL: ${url}`);
    const response = await fetchData(url);
    if (response !== 'OK') {
...
async function main() {
    try {
        ...
        for (const row of json) {
                ...
                if (agentExtension) {
                    ...
                    if (extensionNumber) {
                        if (await isAgentAlreadyInQueue(websiteUrl, apiKey, tenantName, queueId, extensionNumber)) {
                            try {
                                await removeAgentFromQueue(websiteUrl, apiKey, tenantName, queueId, extensionNumber);
                                removedCount++;
                            } catch (error) {
                                console.error(`Błąd podczas usuwania agenta ${agentExtension} z kolejki ${queueName}:`, error);
                            }
                        } else {
                            console.log(`Agent ${extensionNumber} nie znajduje się w kolejce ${queueName}`);
                        }
                    } else {
            ...
            updateProgress(Math.round(((removedCount) / json.length) * 100));
        }
